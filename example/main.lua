...
function love.load()
    ...
    example1box = Text.new(
        "left",
        { color = { 1, 1, 1, 1 }, shadow_color = { 0.5, 0.5, 1, 0.4 }, keep_space_on_line_break = true, default_underline_position = -2, default_strikethrough_position = 1 }
    )
    example1box:send(example.long_text_block, 320 * 4, true)
    -- Example 2 // Golden Apple Font // Earthbound Style Box
    example2box = Text.new(
        "left",
        { color = { 1, 1, 1, 1 }, shadow_color = { 0.5, 0.5, 1, 0.4 }, font = Fonts.golden_apple, keep_space_on_line_break = true, character_sound = true, adjust_line_height = -3 }
    )
    ...
    example3box:send(
        "I am a very cute [color=#00ff00]green frog[/color]. Would you like to eat dinner with me? It's [rainbow][bounce]fresh[/bounce] [u]fly[/u] [shake]soup[/shake]![/rainbow]",
        316,
        false
    )
    --Example 4 // Earth Illusion // Bubble Style Box
    example4box =
        Text.new("left", { color = { 0, 0, 0, 0.95 }, shadow_color = { 0.5, 0.5, 1, 0.4 }, font = Fonts.earth_illusion, keep_space_on_line_break = true, adjust_line_height = -2 })
    ...
    example6box:send(
        "Oh wow, you found the [bounce][rainbow]high-res[/rainbow][/bounce] text! [icon=1][icon=2][icon=3][icon=4] [icon=5][icon=6][icon=7][icon=8] [icon=9][icon=10][icon=11][icon=12][/]",
        320 * 4 - 16,
        true
    )
    ...
    example8box:send(
        "[function=example.bop=true]Did you hear about the [color=#FF0000]bad puns?[/color][pause=0.5] You did?![pause=0.5] That's [color=#FFFF00]great[/color][pause=0.8]!  [shake]Now I don't have to tell you about them![/shake][pause=1][function=example.bop=false][audio=sfx=laugh]",
        320 - 80,
        false
    )
    --Example 9 // Comic Neue / Pocket Monsters Scroll Style
    example9box = Text.new(
        "left",
        {
            color = { 0.9, 0.9, 0.9, 0.95 },
            shadow_color = { 0.5, 0.5, 1, 0.4 },
...
function love.keypressed(key, scancode, isrepeat)
    if key == "space" then
        example.display_mode = example.display_mode + 1
    end
    if example.display_mode > example.maxmodes then
    ...
    if example.display_mode == 7 then
        if key == "r" then
            example7box:send(
                "[warble=-5][textspeed=0.02][image=witch][pad=32]There's something I have to say,[pause=0.7] [warble=5]this witch will save the day!",
                320 * 4 - 16,
                false
            )
        end
